...
PIXI.Loader.shared
  .add(
    "https://raw.githubusercontent.com/isladjan/jsonFiles/main/fire1_sc_biegxd.json"
  )
  .load(jsonLoad);
...
const videoTexture = PIXI.Texture.from("https://i.imgur.com/DLyEGUE.mp4");
...
function setupElements() {
  ...
  for (let i = 1; i < amount + 1; i++) {
    ...
    if (app.screen.height < 350)
      scene.children[i - 1].height = element_height / 1.8;
    else if (app.screen.height > 350 && app.screen.height < 550)
      scene.children[i - 1].height = element_height;
    else scene.children[i - 1].height = element_height * 1.5;

    if (app.screen.width < 1000)
      scene.children[i - 1].width = element_width / 1.7;
